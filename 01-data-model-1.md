...
-- External ingestion. Data always lands here, in the native schema, before anything else touches it —
-- the app never reads live from an external system at request time.
create table integrations (
  id uuid primary key default gen_random_uuid(),
  org_id uuid references organizations(id),
  provider text not null,          -- 'excel_import', 'microsoft365', 'salesforce', 'erp', ...
  config jsonb default '{}',
  connected boolean default false, -- false = unused option, not a missing requirement
...
create table import_batches (
  id uuid primary key default gen_random_uuid(),
  org_id uuid references organizations(id),
  source text not null,              -- 'excel' (day-one, no integration needed) | 'api'
  integration_id uuid references integrations(id),
  column_mapping jsonb not null,     -- source column name -> internal field key
  row_count int,
...
## Notes
- **Supabase Postgres is the single system of record.** Every table above is native storage the app owns outright. External sources — Excel today, APIs/Microsoft 365/Salesforce/ERP later — only ever write *into* these tables via `import_batches`; the app is fully functional with zero integrations connected.
